...
# ! Do this function provide ad-hoc solution of backdoor attack?
def gen_input(args, datareader, bkd_gids):
    """ Prepare inputs for GTN, topology input and feature input together.

    Parameters
    ----------
    args : argparse.Namespace
        Arguments from command line. Control adaptive-input type.
    ...
    -------
    Dict[int, Tensor]
        Dict of adjacency matrix, key is graph id, value is input tensor.

    Dict[int, Tensor]
        Dict of feature matrix, key is graph id, value is input tensor.
    """
    As, Xs = {}, {}
    ...
    for gid in bkd_gids:
        if gid not in As:
            As[gid] = torch.tensor(datareader.data['adj_list'][gid], dtype=torch.float)

        if gid not in Xs:
            Xs[gid] = torch.tensor(datareader.data['features'][gid], dtype=torch.float)
