...
def GetFinalFeatures(args: argparse.Namespace, data):
    """ Construct features for each graph instance, may comes from 3 parts.
    ...
    # In each graph sample, treat node labels (if have) as feature for one graph.
    n_edges: List[int] = []         # Number of edges for each graph
    nlabels: List[np.array] = []    # Node labels, grouped by graph
    degrees: List[np.array] = []    # Node degrees, grouped by graph
    for sample_id, adj in enumerate(data['adj_list']):
        N = len(adj)  # number of nodes
        n = np.sum(adj)  # total sum of edges
    ...
    final_features: List[np.ndarray] = [] # Node features, grouped by graph
    ...
    for sample_id, adj in enumerate(data['adj_list']):
        ...
        # among nodes within a graph. Each position in is 0/1 to show
        ...
        # `use_nlabel_asfeat` is a flag to indicate whether use node labels as feature
        # by default: False.
        ...
        # `use_org_node_attr` is a flag to indicate whether use original node features
        # by default: False.
        if args.use_org_node_attr:
        ...
        # `use_degree_asfeat` is a flag to indicate whether use node degree as feature
        # by default: False.
